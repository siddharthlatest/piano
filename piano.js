 ...
 */
    ...
    // Setup keys!
    //
    Appbase.credentials("helloworld", "d1376babbe1de33768429614410b0cac");
    var ab = Appbase.create("misc", "piano");
    var notesOffset = 0;
    ...
    function isModifierKey(evt) {
        return evt.metaKey || eval(evt.shiftKey) || evt.altKey;
    }

    var playKey = function(keyCode) {
        keytrigger(keyCode);
    }

    ab.on('properties', function(err, abref, snapshot) {
        var obj = snapshot.properties();        
        var array = [];
        var debugTimediff = [];
        for (keyCode in obj) {
            setTimeout(playKey(keyCode), obj[keyCode]);
            array.push(keyCode)
            debugTimediff.push(obj[keyCode]);
        }
        if (array.length !== 0) {
            console.log(array);
            console.log(debugTimediff);
            ab.removeData(array)
        }
    });

    var keytrigger = function(keyCode, evt) {
